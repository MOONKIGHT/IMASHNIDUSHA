...
diana.addqueendiacmd({ queeendiacmd: 'song ?(.*)', fromMe: SKS, desc: Lang.SONG_DESC }, (async(message, match) => {
    ...
    ffmpeg(stream)
        ...
        .on('end', async() => {
            ...
            await message.sendMessage(Buffer.from(iavatar.data), MessageType.image, { mimetype: Mimetype.jpg, thumbnail: thumb, caption: msg, quoted: message.data });
...
//normal mode
//with info
    diana.addqueendiacmd({ queeendiacmd: 'song ?(.*)', fromMe: KSK, deldiacmd: false, noinmenu:true,  deldiacmd: false}, async (message, match) => {
        const linkk = match[1]
        ...
        diana.addqueendiacmd({ queeendiacmd: 'song ?(.*)', fromMe: true, deldiacmd: false, noinmenu:true,  deldiacmd: false}, async (message, match) => {
        const linkk = match[1]
        if (!linkk) return await message.client.sendMessage(message.jid,YT_NEED,MessageType.text)
        await message.client.sendMessage(message.jid,config.SONGD,MessageType.text, {quoted: message.data});
        await axios
